...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None, initial: dict | None = None, mode: str = "init"):
        ...
        # Whisper: dropdown with predefined sizes + German-finetuned models.
        # The German models are CTranslate2 conversions that faster-whisper loads
        # directly by HuggingFace repo id — far better German accuracy than the
        # generic multilingual sizes (WER ~2.6% vs. weak "base").
        self._whisper_combo = QComboBox()
        self._whisper_combo.setFixedHeight(28)
        self._whisper_combo.setStyleSheet(_COMBO_STYLE)
        _WHISPER_MODELS = [
            ("tiny",     "tiny  — schnell, schwach"),
            ("base",     "base  — Standard"),
            ("small",    "small"),
            ("medium",   "medium"),
            ("large-v3", "large-v3  — bestes Multilingual"),
            ("jimmymeister/whisper-large-v3-turbo-german-ct2",
             "🇩🇪 large-v3-turbo German  — empfohlen (frei, schnell+genau)"),
            ("TheTobyB/whisper-large-v3-turbo-german-ct2",
             "🇩🇪 large-v3-turbo German (Alternative)"),
        ]
        for _val, _disp in _WHISPER_MODELS:
            self._whisper_combo.addItem(_disp, userData=_val)
        _cur_model = _init.get("stt_model", "base")
        _idx = self._whisper_combo.findData(_cur_model)
        if _idx < 0:                       # custom/unknown value → show it as-is
            self._whisper_combo.addItem(_cur_model, userData=_cur_model)
            _idx = self._whisper_combo.findData(_cur_model)
        ...
        tts_row, self._tts_btns = _toggle_row(
            [("edgetts","🔈 EdgeTTS"), ("kokoro","🤖 Kokoro"),
             ("ttsserver","🐳 DE-Server"), ("elevenlabs","⚡ ElevenLabs")],
        ...
        # EdgeTTS: quick-pick dropdown of common voices (German first, no download).
        # Selecting an entry fills the text field above, which stays editable so
        # any other Edge voice can still be typed in manually.
        self._edge_combo = QComboBox()
        self._edge_combo.setFixedHeight(28)
        self._edge_combo.setStyleSheet(_COMBO_STYLE)
        _EDGE_VOICES = [
            ("de-DE-KatjaNeural",     "de-DE-Katja  — DE-F (empfohlen)"),
            ("de-DE-ConradNeural",    "de-DE-Conrad  — DE-M"),
            ("de-DE-AmalaNeural",     "de-DE-Amala  — DE-F"),
            ("de-DE-KillianNeural",   "de-DE-Killian  — DE-M"),
            ("de-DE-SeraphinaMultilingualNeural", "de-DE-Seraphina  — DE-F multilingual"),
            ("de-DE-FlorianMultilingualNeural",   "de-DE-Florian  — DE-M multilingual"),
            ("de-AT-IngridNeural",    "de-AT-Ingrid  — Österreich-F"),
            ("de-AT-JonasNeural",     "de-AT-Jonas  — Österreich-M"),
            ("de-CH-LeniNeural",      "de-CH-Leni  — Schweiz-F"),
            ("de-CH-JanNeural",       "de-CH-Jan  — Schweiz-M"),
            ("en-US-GuyNeural",       "en-US-Guy  — EN-M"),
            ("en-US-AriaNeural",      "en-US-Aria  — EN-F"),
            ("en-GB-RyanNeural",      "en-GB-Ryan  — UK-M"),
        ]
        for val, display in _EDGE_VOICES:
            self._edge_combo.addItem(display, userData=val)
        _cur_edge = _init.get("tts_voice", "en-US-GuyNeural")
        for i in range(self._edge_combo.count()):
            if self._edge_combo.itemData(i) == _cur_edge:
                self._edge_combo.setCurrentIndex(i)
                break
        self._edge_combo.activated.connect(
            lambda _i: self._tts_voice_input.setText(self._edge_combo.currentData() or "")
        )
        self._edge_combo.setVisible(False)  # shown only when EdgeTTS active
        voice_row.addWidget(self._edge_combo)
        ...
        # TTS-Server URL — only visible when DE-Server (HTTP) is selected
        self._tts_server_widget = QWidget()
        self._tts_server_widget.setStyleSheet("background: transparent;")
        ts_row = QHBoxLayout(self._tts_server_widget)
        ts_row.setContentsMargins(0, 0, 0, 0)
        ts_row.setSpacing(5)
        ts_row.addWidget(_lbl("Server-URL:", 7, col=C.TEXT_MED,
                               align=Qt.AlignmentFlag.AlignRight))
        self._tts_server_url_input = _input("http://localhost:8080")
        self._tts_server_url_input.setText(
            _init.get("tts_server_url", "http://localhost:8080"))
        ts_row.addWidget(self._tts_server_url_input)
        layout.addWidget(self._tts_server_widget)
    ...
    def _update_tts_ui(self, key: str) -> None:
        ...
        # EdgeTTS additionally shows a quick-pick dropdown next to the text field.
        ...
        if hasattr(self, "_edge_combo"):
            self._edge_combo.setVisible(is_edge)
        ...
        if key == "elevenlabs":
            ...
            if hasattr(self, "_tts_voice_input"):
                self._tts_voice_input.setPlaceholderText(
                    "dm_martin  /  df_victoria  /  df_eva  …  (Container-Stimme)"
                )
        else:  # edgetts
            self._voice_lbl.setText("Voice:")
            if hasattr(self, "_tts_voice_input"):
                self._tts_voice_input.setPlaceholderText(
                    "de-DE-KatjaNeural  /  de-DE-ConradNeural  /  en-US-GuyNeural  …"
                )
                # Reset a stale voice from another engine (e.g. Kokoro's
                # "af_heart") — EdgeTTS only accepts xx-XX-*Neural and would crash.
                import re as _re
                _cur = self._tts_voice_input.text().strip()
                if _cur and not _re.match(r"^[a-z]{2,}-[A-Z]{2,}-.+Neural$", _cur):
                    self._tts_voice_input.setText("de-DE-KatjaNeural")
        ...
        if hasattr(self, "_tts_server_widget"):
            self._tts_server_widget.setVisible(key == "ttsserver")
    ...
    def _submit(self):
        ...
        # STT model: combo for Whisper (value held in userData), text for Vosk
        if self._sel_stt == "whisper":
            stt_model = self._whisper_combo.currentData() or "base"
        else:
        ...
        if self._sel_tts == "kokoro":
            ...
            _voice_default = "dm_martin" if self._sel_tts == "ttsserver" else "en-US-GuyNeural"
            tts_voice = self._tts_voice_input.text().strip() or _voice_default
            tts_speed = "1.0"
        ...
        cfg = {
            ...
            "tts_server_url":     self._tts_server_url_input.text().strip() or "http://localhost:8080",
        }
