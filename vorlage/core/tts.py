...
def _enable_hf_online() -> None:
    ...
    library caches `constants.HF_HUB_OFFLINE = True` at import time.  Popping the
    env var afterwards is too late — the cached constant still blocks downloads
    with OfflineModeIsEnabled.  Overwrite both the env vars and the constant so
...
# NOTE: Kokoro-82M has NO German voice.  Supported languages only:
#   a American-EN · b British-EN · e Spanish · f French · h Hindi ·
#   i Italian · j Japanese · p Brazilian-Portuguese · z Mandarin.
# For German speech output use EdgeTTS (de-DE-* voices) instead.
_KOKORO_LANG_CODES = {
...
class OpenAISpeechTTSEngine:
    """TTS via any OpenAI-compatible /v1/audio/speech HTTP endpoint.

    Lets a heavy or OS-specific engine run out-of-process — e.g. a German
    Kokoro Docker container (Godelaune/Kokoro-82M-ONNX-German-Martin) which
    keeps espeak-ng and all model dependencies inside Linux.  The app only
    makes an HTTP call and plays the returned audio, so the host (Windows)
    stays clean.  Also works with any other OpenAI-TTS-compatible server.
    """
    ...
    def speak(self, text: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"TTS server not reachable at {self.base_url}.\n"
                "Is the German Kokoro Docker container running?  Start it with:\n"
                "    docker compose up -d\n"
                f"Details: {e}"
            ) from e
        _play_audio_bytes(resp.content)
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    elif engine_name == "ttsserver":   # OpenAI-compatible HTTP server (e.g. German Kokoro Docker)
        engine = OpenAISpeechTTSEngine(
            base_url = config.get("tts_server_url",    "http://localhost:8080"),
            voice    = config.get("tts_voice",         "dm_martin"),
            model    = config.get("tts_server_model",  "kokoro"),
            fmt      = config.get("tts_server_format", "wav"),
            api_key  = config.get("tts_server_key",    ""),
        )
    else:   # edgetts (default)
        voice = config.get("tts_voice", "en-US-GuyNeural")
        # Guard against a stale voice from another engine (e.g. Kokoro's
        # "af_heart") leaking into EdgeTTS, which only accepts xx-XX-*Neural.
        import re
        if not re.match(r"^[a-z]{2,}-[A-Z]{2,}-.+Neural$", voice):
            print(f"[TTS] '{voice}' is not a valid EdgeTTS voice — using de-DE-KatjaNeural.")
