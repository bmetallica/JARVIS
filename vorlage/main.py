...
# Set of valid tool names + a matcher for tool calls the model emits as plain
# TEXT (e.g.  open_app {"app_name": "Editor"}  ) instead of a structured
# tool_call.  Smaller local models (qwen2.5-7b via llama.cpp) do this
# intermittently, especially once tool history is in the context.
_TOOL_NAME_SET = {d["name"].lower() for d in TOOL_DECLARATIONS}
# name  +  {json}   or   name(  {json}  )   — JSON object only, whole string
_TOOL_TEXT_RE = re.compile(r'^\s*([A-Za-z_]\w*)\s*[:(]?\s*(\{.*\})\s*\)?\s*$', re.DOTALL)
# Lightweight "this sentence is the start of a text tool call" check (used to
# avoid speaking the raw JSON while streaming).
_TOOL_TEXT_PREFIX_RE = re.compile(r'^([A-Za-z_]\w*)\s*[{(]')
...
class JarvisLocal:
    ...
    def _do_reconfigure(self, new_config: dict) -> None:
        ...
        if old_stt_engine == new_stt_engine:
            try:
                ...
                stt_model    = new_config.get("stt_model", "base")
                if stt_language == "auto" and "german" in stt_model.lower():
                    stt_language = "de"
                if new_stt_engine == "vosk":
                    from core.stt import VoskSTT
                    self._stt = VoskSTT(new_config.get("vosk_model_path"), language=stt_language)
                elif new_stt_engine in ("whisper_server", "remote"):
                    from core.stt import RemoteWhisperSTT
                    self._stt = RemoteWhisperSTT(
                        base_url = new_config.get("stt_server_url", "http://localhost:8001"),
                        model    = stt_model,
                        language = stt_language,
                        api_key  = new_config.get("stt_server_key", ""),
                    )
                else:
    ...
    def _looks_like_tool_text(text: str) -> bool:
        """True if a streamed chunk is the start of a text-emitted tool call.

        Used to avoid speaking raw JSON like  open_app {"app_name": "Editor"}
        when the model emits a tool call as content instead of a tool_call.
        """
        m = _TOOL_TEXT_PREFIX_RE.match(text.strip())
        return bool(m) and m.group(1).lower() in _TOOL_NAME_SET

    @staticmethod
    def _parse_text_tool_call(content: str) -> dict | None:
        """Parse a tool call the model emitted as plain text into a synthetic
        tool_call dict, or return None if the content isn't a tool call."""
        if not content:
            return None
        m = _TOOL_TEXT_RE.match(content.strip())
        if not m:
            return None
        ...
        if not isinstance(args, dict):
            return None
        return {"id": "", "function": {"name": name, "arguments": args}}
    ...
    def _process_message(self, user_text: str) -> None:
        ...
        # Critical for OpenAI-compatible servers (LM Studio, llama.cpp):
        # a "tool" message MUST be preceded by the "assistant" message whose
        # tool_calls it answers.  Slicing the window can orphan a leading tool/
        # assistant-tool_calls message → the server rejects the whole request
        # with HTTP 500.  Drop leading messages until the window starts cleanly
        # on a user turn, which keeps every assistant→tool pairing intact.
        ...
        for _round in range(MAX_TOOL_ROUNDS):
            ...
            try:
                for event in call_llm_stream(messages, OLLAMA_TOOLS):
                    if event["type"] == "sentence":
                        ...
                        # the next one.  Skip chunks that are actually a tool
                        # call emitted as text — we don't want to read JSON aloud.
                        if self._looks_like_tool_text(event["text"]):
                            continue
            ...
            # ── Fallback: model emitted a tool call as plain text ────────────
            # (e.g.  open_app {"app_name": "Editor"}  instead of a tool_call).
            # Convert it so the normal tool-execution path below runs it.
            if not final_tool_calls:
                _parsed = self._parse_text_tool_call(final_content)
                if _parsed:
                    final_tool_calls = [_parsed]
                    final_content    = ""
            ...
            for tc in final_tool_calls:
                ...
                # Cap tool output kept in the message history.  A huge result
                # (file dump, web page, screen text) can blow past the model's
                # context window and make the server return HTTP 500.  6000 chars
                # is plenty for a 7B model to work with; the full result is still
                # used for speaking via _tool_results.
                _content = "Done." if result == "__SILENT__" else str(result)
                if len(_content) > 6000:
                    _content = _content[:6000] + "\n…[gekürzt]"
                tool_msg: dict = {
                    "role":    "tool",
                    "content": _content,
                }
    ...
    def _listen_whisper(self) -> None:
        ...
        if self._stt is None:
            self.ui.write_log(
                "ERR: Speech recognition is offline — the Whisper model never loaded "
                "(see the STT error above). This is NOT a microphone problem.\n"
                "     Fix: fully close and restart MARK XL so the model can download "
                "once (internet required the first time)."
            )
            return
    ...
    def _listen_vosk(self) -> None:
        ...
        if self._stt is None:
            self.ui.write_log(
                "ERR: Speech recognition is offline — the Vosk model never loaded "
                "(see the STT error above). This is NOT a microphone problem.\n"
                "     Fix: fully close and restart MARK XL."
            )
            return
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # A German-finetuned Whisper model works best pinned to German —
            # don't leave it on language auto-detect.
            if stt_language == "auto" and "german" in stt_model.lower():
                stt_language = "de"
            ...
            def _do_stt():
                try:
                    ...
                    elif stt_engine in ("whisper_server", "remote"):
                        from core.stt import RemoteWhisperSTT
                        self._stt = RemoteWhisperSTT(
                            base_url = self._config.get("stt_server_url", "http://localhost:8001"),
                            model    = stt_model,
                            language = stt_language,
                            api_key  = self._config.get("stt_server_key", ""),
                        )
                    else:
                        from core.stt import WhisperSTT
                        self._stt = WhisperSTT(stt_model, language=stt_language)
